...
export const registerUser = TryCatch(async (req, res) => {
    ...
    if (!validation.success) {
        ...
        return res.status(400).json({ message: zodError.issues[0].message })
    }
    ...
    console.log("Setting Redis Key:", verifyKey);
...
export const loginUser = TryCatch(async (req, res) => {
  ...
  await redisClient.set(otpKey, JSON.stringify(otp), {
    EX: 300,//for 5 miute means 300 seconds
  });
  ...
  await redisClient.set(rateLimitKey, "true", {
    EX: 60,//for 1 miute means 60 seconds
  });
